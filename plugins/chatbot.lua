...
if matches[1] == "بای" then
     ...
     if msg.to.type == 'channel' or 'chat' then
	  ...
	  if matches[1] == "😂" then
      if msg.to.type == 'channel' or 'chat' then
           local answers = {'😂😂😂','وای دلم😂','شما چقد خوش خنده ای عشقم😂','جوون تو فقط بخند 😂😂','اه بسه دیگه چقد میخندی','یکی اینو ببره تا از خنده غش نکرده😂','اوووف عاشق خنده هاتم😂','جدی بسه😂','خنده هات خیلی زیباست عشقم😂😂','لطقا تمومش کن','دیشب تو دریاچه نمک بودی انگار','بسه دیگه','کافیه برای چی میخندی','مسخره بازیا رو تموم کن','مسخره کردی منو؟'}
            return answers[math.random(#answers)]
      end
	   end
